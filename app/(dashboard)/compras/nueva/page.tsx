  ...
  const handleGuardarCompra = async () => {
    ...
    try {
      ...
      for (const item of cart) {
        ...
        const { error: dError } = await supabase.from('DetalleCompra').insert([detallePayload]);
        if (dError) throw dError;

        // Actualizar Stock y Precio de Compra
        const { data: currentProd } = await supabase.from('Producto').select('stock_actual').eq('id', item.id).single();

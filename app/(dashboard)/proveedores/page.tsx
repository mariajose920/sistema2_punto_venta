          ...
          <div key={p.id_proveedor} className="bg-white dark:bg-gray-800 p-8 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all group">
            <div className="flex justify-between items-start mb-6">
              <div className="min-w-0">
                <h2 className="text-xl font-black text-gray-900 dark:text-white truncate">{p.nombre_empresa}</h2>
            ...
            <div className="grid grid-cols-1 gap-4 border-t border-gray-50 dark:border-gray-700 pt-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <p className="text-sm font-bold text-gray-700 dark:text-gray-300 truncate">{p.correo_}</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-lg">📍</span>
                <p className="text-xs text-gray-400 font-medium truncate">{p.direccion}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 w-full max-w-2xl rounded-[2.5rem] p-10 shadow-2xl max-h-[80vh] overflow-auto animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              {compras.map(c => (
                <div key={c.id_compra} className="p-6 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-gray-100 dark:border-gray-800 flex justify-between items-center group hover:border-emerald-200 transition-all">
                  <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase mb-1 tracking-widest">FECHA: {new Date(c.fecha_compra).toLocaleDateString()}</p>

    ...
    const fetchData = async () => {
      try {
        ...
        const [pRes, cRes, promoRes] = await Promise.all([
          supabase.from('Producto').select('*').gt('stock_actual', 0),
  ...
  const handleFinalizarVenta = async () => {
    ...
    try {
      ...
      for (const item of cart) {
        ...
        const { error: dError } = await supabase.from('DetalleVenta').insert([detallePayload]);
      ...
      if (selectedClient) {
        ...
        if (paymentMethod === 'fiado') {
          ...
          if (totalFinal > 0) {
            ...
            await supabase.from('Credito').insert([creditoPayload]);
          }
        }

        const { error: cliUpdateError } = await supabase
          .from('Cliente')
          .update({ 
